...
`gametools.network`的服务器可能出现了故障返回了旧的数据，这时候只能等待服务器恢复正常后再查询。

### 4. 真实KD和真实KPM是什么意思？

真实KD和真实KPM是除去AI击杀后的KD和KPM。

### 5. 真实击杀比例是什么意思？

真实击杀比例是击杀的真实玩家占总击杀数的比例。

### 6. 为什么软件内的游戏时长与Steam/EA App等平台的游戏时长不一致？

`gametools.network`只记录对局内的游戏时长，且门户或官方AI房并不计入游戏时长。
...
`gametools.network`并不提供玩家等级的数据，所以无法显示。

### 9. UID怎么获取？

只能先用昵称查询，然后在查询结果中查看UID。

### 10. 我被BFBan了怎么办？

请自行前往[BFBan官网](https://bfban.com/)查看原因，本应用不对该结果负责！
